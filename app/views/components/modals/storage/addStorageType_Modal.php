<div id="addStorageType" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full animate__animated animate__fadeInDownBig md:mx-2">
        ...
        <div class="relative rounded-lg shadow-sm bg-gray-900">
            ...
            <div class="flex items-center p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                <div class="flex items-center">
                    ...
                    <div class="flex-col">
                        <span class="text-xl font-medium text-white">Añadir Nuevo Tipo</span>
                        <div class="flex items-center gap-x-1">
                            <span class="text-xs font-medium px-1.5 py-0.5 rounded-sm bg-green-900 text-green-300">Ficha de Registro</span>
                        </div>
                    </div>
                </div>
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="addStorageType">
            ...
            <form action="<?= $AjaxRoutes['storage'] ?>" class="AjaxForm" method="POST" autocomplete="OFF">
                ...
                <div class="w-full flex items-center justify-end p-4 md:p-5 space-x-3 rtl:space-x-reverse border-t border-gray-200 rounded-b">
                    <button data-modal-hide="addStorageType" type="button" class="AjaxForm px-3 py-2 text-sm font-medium text-center inline-flex items-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 mr-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('[data-modal-target="addStorageType"]').forEach(function(btn) {
        ...
        function clearModal() {
            const form = document.querySelector('#addStorageType form');
            if (!form) return;
            form.reset();
        }
    });
